...
function showAuthScreen() { appWrapper.classList.add('is-transparent'); setTimeout(() => { appWrapper.classList.add('hidden'); authContainer.classList.remove('hidden'); requestAnimationFrame(() => { authContainer.classList.remove('is-transparent'); }); }, TRANSITION_DURATION); }
function showAppScreen() {
    ...
    setTimeout(() => {
        ...
        // **FIX**: Load data and initialize UI AFTER the app is visible
        if (isOfflineMode) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- INITIAL APP LOAD ---
    function initializeApp() { if (localStorage.getItem('pinkDaysOfflineMode') === 'true') { isOfflineMode = true; currentUser = null; showAppScreen(); addOfflineExitButton(); } else { onAuthStateChanged(auth, handleAuthState); } }
    ...
    async function handleAuthState(user) {
        if (user) {
            ...
            showAppScreen();
            // **FIX**: The line that was here is now in showAppScreen()
            addLogoutButton();
        } else {
            currentUser = null;
            if (!isOfflineMode) {
    ...
    function initializeAppUI() {
        ...
        // This can be a simple call now, but the small timeout is harmless and safe
    ...
    function switchTab(tabName) { app.tabPanes.forEach(function(pane) { pane.classList.add('hidden'); }); document.getElementById(tabName + '-tab').classList.remove('hidden'); const nav = document.querySelector('nav.flex'); const activeTab = document.querySelector(`.tab-btn[data-tab="${tabName}"]`); app.tabButtons.forEach(function(btn) { btn.classList.toggle('active', btn.dataset.tab === tabName); }); if (nav && activeTab) { const clientWidth = activeTab.clientWidth; if (clientWidth === 0) { console.warn("switchTab called when tab width is 0."); return; } const offsetLeft = activeTab.offsetLeft; nav.style.setProperty('--indicator-left', `${offsetLeft}px`); nav.style.setProperty('--indicator-width', `${clientWidth}px`); } localStorage.setItem('pinkDaysLastTab', tabName); }
